...
export function MetaView({ onSelectHero }) {
  ...
  return (
    <>
      ...
      {BANS.map(h => (
        <button key={h.n} type="button" style={{ ...s.cd2, ...s.btnReset, display: "flex", alignItems: "center", gap: 10 }} onClick={() => open(h.n)}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700 }}>{h.n} <span style={{ color: h.tr === "↑" ? P.nG : h.tr === "↓" ? P.red : P.t3 }}>{h.tr}</span></div>
      ...
      {RISING.map(h => (
        <button key={h.n} type="button" style={{ ...s.cd2, ...s.btnReset, borderLeft: `3px solid ${P.nG}` }} onClick={() => open(h.n)}>
          <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 13, fontWeight: 700 }}>{h.n}</span><span style={{ fontSize: 11, color: P.nG, fontWeight: 700 }}>{h.ch}</span></div>
          <div style={{ fontSize: 11, color: P.t2, marginTop: 2 }}>{h.w}</div>
        </button>
      ))}
      <div style={s.sc}>📉 Falling</div>
      {FALLING.map(h => (
        <button key={h.n} type="button" style={{ ...s.cd2, ...s.btnReset, borderLeft: `3px solid ${P.red}` }} onClick={() => open(h.n)}>
          <div style={{ display: "flex", justifyContent: "space-between" }}><span style={{ fontSize: 13, fontWeight: 700 }}>{h.n}</span><span style={{ fontSize: 11, color: P.red, fontWeight: 700 }}>{h.ch}</span></div>
          <div style={{ fontSize: 11, color: P.t2, marginTop: 2 }}>{h.w}</div>
        </button>
      ))}
      ...
      {LANES.map(ln => (
        <div key={ln.l} style={{ ...s.cd2, cursor: "default" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 0, marginTop: 4 }}>{ln.top.map((n, i) => <button key={n} type="button" style={s.ch(i === 0 ? P.gold : P.blue)} onClick={() => open(n)}>{i === 0 ? "👑 " : ""}{n}</button>)}</div>
