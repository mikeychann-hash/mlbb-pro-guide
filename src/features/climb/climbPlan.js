...
const wrOf = (h) => (Number.isFinite(h.wr) ? h.wr : 0);
const byStrength = (a, b) => (TIER_RANK[b.t] || 0) - (TIER_RANK[a.t] || 0) || (wrOf(b) - wrOf(a));
...
export function climbPlan(heroes, lanes = [], favorites = []) {
  ...
  return {
    ...
    learn: nonFav.filter((h) => !h.pending && (TIER_RANK[h.t] || 0) >= 4).sort(byStrength).slice(0, 4),
    banThreats: pool.slice().sort((a, b) => (b.br || 0) - (a.br || 0)).filter((h) => (h.br || 0) > 0).slice(0, 4),
    avoid: pool.filter((h) => !h.pending && (TIER_RANK[h.t] || 0) <= 2).sort((a, b) => wrOf(a) - wrOf(b)).slice(0, 4),
  };
}
